...
#' @description  \code{dPLD} gives the probability density function of Power Lindley Distribution.
#' @details
#'  The probability density function (PDF) and cumulative distribution function (CDF) are as follows:
#' \deqn{f\left( x\right) =\frac{\alpha \beta ^{2}}{\beta +1}\left( 1+x^{\alpha}\right) x^{\alpha -1}\exp \left( -\beta x^{\alpha }\right)}
#' \deqn{=zg_{1}\left( t\right) +\left( 1-z\right) g_{2}\left( t\right)}
#'
#' \deqn{F\left( x\right) =P\left( X\leq x\right) =1-\left( 1+zx^{\alpha }\right)
...
#'                                               \right) \right) +1+\beta }{\beta }\right\} ^{\frac{1}{\alpha }}}
#' ,where
...
#' x^{\alpha }\right)}
#' \eqn{\alpha>0} is a shape parameter, \eqn{\beta>0} is a scale parameter, \eqn{0<u<1} and \eqn{W} is Lambert W function.
#'
#' Let \eqn{c} be the cut-off point, \eqn{X_{1}\sim PL\left( \alpha_{1},\beta_{1}\right) }
#' and \eqn{X_{2}\sim PL\left( \alpha_{2},\beta_{2}\right) } with \eqn{\alpha_{2}>\alpha_{1}}.
#' \eqn{F_{1}} and \eqn{F_{2}} are the cumulative distribution functions related to \eqn{X_{1}} and \eqn{X_{2}}, respectively.
#' In that case, the 1-specificity (False Positive Rate,FPR) and sensitivity (True Positive Rate,TPR) are given by,
#' \deqn{FPR =P\left( X_{1}>c\right) =1-P\left( X_{1}\leq c\right) =1-F_{1}\left(
#' c\right)}
#' \deqn{TPR =1-F_{2}\left( Q_{1}\left( 1-FPR\right) \right)}
#' and Receiver Operating Characteristic (ROC) curve can be expressed as follows:
#' \deqn{ROC =\left\{\left(r,( 1-F_{2}\left( Q_{1}\left(
#' 1-r; \boldsymbol{\theta }\right) \right), r\in \left( 0,1\right) \right)\right\}},
#' where \eqn{\boldsymbol{\theta }=\left( \alpha _{1},\alpha _{2},\beta _{1},\beta
#' _{2}\right)}.
#'
#'Performance assesments for the ROC analysis are as follows:
#' \deqn{AUC =\int\limits_{0}^{1}ROCdr}
#' \deqn{J\left( c\right)  =\underset{c}{\arg \max }\left\{ \left( 1-F_{2}\left( c\right)
#' \right) +F_{1}\left( c\right) -1\right\}}
#' \deqn{ER\left( c\right)  = \underset{c}{\arg \min } \left\{ \sqrt{\left( 1-F_{1}\left( c\right) \right)
#' ^{2}+\left( F_{2}\left( c\right) \right) ^{2}} \right\}}
#' \deqn{CZ\left( c\right)  = \underset{c}{\arg \max } \left\{ \left( 1-F_{2}\left( c\right) \right) \times F_{1}\left(
#' c\right)  \right\}}
...
#'                 \left( 1-F_{1}\left( c\right) \right) \times \left( 1-F_{2}\left( c\right)
#'                                                                     \right) \right) \right\}}
#',where AUC is area under the ROC curve, J is Youden's J index, ER is the closest to
#'\eqn{\left( 0,1\right)} criteria, CZ is the concordance probability method, IU is index of Union
#'and NI is new index.
#' @return  \code{dPLD} returns a probability density function against the specified value.
#' @references
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{pPLD} gives the cumulative density function of Power Lindley Distribution.
#' @return \code{pPLD} returns a cumulative density function against the specified value.
#' @examples
#' pPLD(c(.5,1,2,3,4),alpha=3,beta=2)
pPLD<-function(x,alpha,beta){
  if(any(alpha<=0)) {stop(paste("alpha value must be greather than 0","\n",""))}
  if(any(beta<=0)) {stop(paste("beta value must be greather than 0","\n",""))}
  cdf<-NULL;for (i in 1:length(x)) {
    ifelse(x[i]>0,
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{qPLD} gives the quantile function of Power Lindley Distribution.
#' @return \code{qPLD} returns a quantile function against the specified value.
#' @examples
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{rPLD} gives random numbers from Power Lindley Distribution.
#' @return \code{rPLD} returns random numbers against the specified n value.
#' @examples
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{plAUC} gives area under curve of Power Lindley Distribution. Area under the ROC curve is obtained.
#' @return \code{plAUC} returns area under ROC curve value.
#' @examples
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{plJ} gives Youden's index for Power Lindley Distribution. Youden's J statistics is a way of summarising the performance of a diagnostic test.
#' @return \code{plJ} returns Youden's index value.
#' @examples
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{plER} gives the Closest to (0,1) Criteria (ER) for Power Lindley Distribution. The Closest to (0,1) Criteria (ER) is a way of summarising the performance of a diagnostic test.
#' @return \code{plER} returns the Closest to (0,1) Criteria (ER) value.
#' @examples
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{plCZ} gives the Concordance Probability Method  for Power Lindley Distribution. The Concordance Probability Method is a way of summarising the performance of a diagnostic test.
#' @return \code{plCZ} returns the Concordance Probability Method  value.
#' @examples
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{plIU} gives the Index of Union  for Power Lindley Distribution. The Index of Union is a way of summarising the performance of a diagnostic test.
#' @return \code{plIU} returns the Index of Union  value.
#' @examples
...
plIU<- function(alpha1,beta1,alpha2,beta2,init=0)
{
  ...
  if(any(beta2<=0)) {stop(paste("beta2 value must be greather than 0","\n",""))}
  iu<- function(c) {
    a<- (base::abs((1-pPLD(c,alpha2,beta2))-plAUC(alpha1,beta1,alpha2,beta2))+base::abs((pPLD(c,alpha1,beta1))-plAUC(alpha1,beta1,alpha2,beta2)))
    return(a)
  }
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{plNI} gives the New Index for Power Lindley Distribution.
#' @return \code{plNI} returns the New Index value.
#' @examples
...
plNI<- function(alpha1,beta1,alpha2,beta2,init=0)
{
  ...
  if(any(beta2<=0)) {stop(paste("beta2 value must be greather than 0","\n",""))}
  ni<- function(c) {
    ni<-((1-pPLD(c,alpha1,beta1))*(1-(1-pPLD(c,alpha2,beta2)))*pPLD(c,alpha1,beta1)*(1-pPLD(c,alpha2,beta2)))+(pPLD(c,alpha1,beta1)*(1-(1-pPLD(c,alpha2,beta2)))*(1-pPLD(c,alpha1,beta1))*(1-pPLD(c,alpha2,beta2)))
    return(-ni)
  }
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{plROC} gives the ROC graph for Power Lindley Distribution.The ROC graph is given by 1-specificity versus sensitivity.
#' @return \code{plROC} returns the ROC graph.
#' @examples
...
plROC<- function(x,y,alpha1,beta1,alpha2,beta2,empirical=TRUE)
{
  ...
  plot(1-r,roc,type="l", lty=2, lwd=2,ylim = c(0,1),xlim= c(0,1),ylab = "Sensitivity",xlab = "1-Specificity",col=1,main="ROC Graph")
  graphics::lines(c(0,1),c(0,1))
  graphics::par(new=T)
  Fn<- stats::ecdf(y)
  roc2<- 1-Fn(stats::quantile(x,r))
  plot(1-r,roc2,type="l",lwd=2,ylim = c(0,1),xlim= c(0,1),ylab = "Sensitivity",xlab = "1-Specificity",col=1,main="ROC Graph")
  graphics::legend('bottomright',inset=0.05,c("Empirical ROC Curve","Fitted ROC Curve"),lty=c(1,2),lwd = c(2,2),col=c(1,1))
...
#' @export
#' @rdname PLindleyROC
#' @description   \code{prfROC} gives sensitivity,specificity and 1-specificity for Power Lindley Distribution.
#' @return \code{prfROC} returns the ROC performance assesments.
#' @examples
